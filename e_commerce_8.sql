create database ecommerce_8;
use ecommerce_8;
-- drop database ecommerce_8;
...
select * from cliente;
select PNome from cliente where CPF=123467809 and  Endereço like 'Street Flowers';

select * from cliente where PNome not like 'JOSH';
...
values ('SmartTV', 'Eletrônico', '4', null),
       ...
       ('Jogo de talheres', 'Domésticos', '1', '1');
...
-- Qual o número total de clientds?
...
select * from vendedor;
select * from vendedor order by idVendedor;
...
inner join  pedido po on f.idFornecedor = p.idProduto;

-- COMO A JUNÇAO DOS DOIS COMANDO FUNCIONAM?

select * from pedido;
select * from pedido order by idPedido;

-- COMO ORDENAR OS DADOS DO CPF EM ORDEM CRESCENTE?
